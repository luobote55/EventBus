...
func TestNewServer(t *testing.T) {
	serverBus := NewServer(":2010", "/_server_bus_", NewEventBus())
	serverBus.Start()
	if serverBus == nil || !serverBus.service.started {
		t.Log("NewEventBus server EventBus not created!")
		t.Fail()
	}
	serverBus.Stop()
}
...
func TestNewClient(t *testing.T) {
	clientBus := NewClient(":2015", "/_client_bus_", NewEventBus())
	clientBus.Start()
	if clientBus == nil || !clientBus.service.started {
		t.Log("NewEventBus client EventBus not created!")
		t.Fail()
	}
	clientBus.Stop()
}
...
func TestRegister(t *testing.T) {
	...
	serverBus := NewServer(":2010", serverPath, NewEventBus())
...
func TestPushEvent(t *testing.T) {
	clientBus := NewClient("localhost:2015", "/_client_bus_", NewEventBus())
...
func TestServerPublish(t *testing.T) {
	serverBus := NewServer(":2020", "/_server_bus_b", NewEventBus())
	serverBus.Start()
	...
	clientBus := NewClient(":2025", "/_client_bus_b", NewEventBus())
	clientBus.Start()
